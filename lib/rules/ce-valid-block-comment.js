            ...
            Program() {
                ...
                blockComments.forEach(function forEachBlockComment(comment){
                    if( comment.loc.start.line === comment.loc.end.line ) {
                        ...
                        if( false && comment.value.match(validBlockCommentNoTab) ){
                            /*var tab = comment.loc.start.column + 1;
                            var validBlockComment = new RegExp('^\\*\\s*\\n(\\s{' + tab + '}\\*\\s[^\\n]*\\n)+\\s+$');
                            ...
                            for( var i = 1 ; i < lines.length -1 ; i++ ){
                                if (!lines[i].match(new RegExp('^\\s{' + tab + '}\\*.*$'))) {
                                    context.report({
                                      loc: {
                                        start: {
                            ...
                            if (!(lines[lines.length-1].match(new RegExp('^\\s{' + tab + '}$')))) {
                                context.report({
                                  loc: {
                                    start: {
